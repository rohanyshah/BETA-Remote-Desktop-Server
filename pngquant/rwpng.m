...
pngquant_error rwpng_read_image(FILE *infile, read_info *mainprog_ptr)
{
    ...
    mainprog_ptr->rowbytes = rowbytes = (unsigned int)png_get_rowbytes(png_ptr, info_ptr);
    ...
    // Commented out because it was giving compile errors; I don't think this line actually does anything anyway.
...
#pragma mark - Pimped functions

void read_png_buffer(png_structp png_ptr, png_bytep buffer, png_size_t length);
void read_png_buffer(png_structp png_ptr, png_bytep buffer, png_size_t length) {
    void *data = png_get_io_ptr(png_ptr);
    memcpy(buffer, data, length);
    
    png_set_read_fn(png_ptr, data+length, read_png_buffer);
}

void write_png_buffer(png_structp png_ptr, png_bytep data, png_size_t length);
void write_png_buffer(png_structp png_ptr, png_bytep data, png_size_t length) {
    NSMutableData **buffer_addr = (NSMutableData **)png_get_io_ptr(png_ptr);

    if((*buffer_addr) == nil) {
        (*buffer_addr) = [[[NSMutableData alloc] initWithLength:0] autorelease];
    }

    [(*buffer_addr) appendBytes:data length:length];
}

void flush_png_buffer(png_structp png_ptr);
void flush_png_buffer(png_structp png_ptr) {
    // noop
}
...
pngquant_error rwpng_read_image_from_buffer(void *indata, read_info *mainprog_ptr) {
    ...
    //png_init_io(png_ptr, infile); // modification: not using file IO anymore!
    png_set_read_fn(png_ptr, indata, read_png_buffer);
...
pngquant_error rwpng_write_image_init_to_buffer(NSData **buffer, write_info *mainprog_ptr)
{
    ...
    png_set_write_fn(png_ptr, buffer, write_png_buffer, flush_png_buffer);
